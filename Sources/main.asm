...
; ---------------------
              ORG   $3800
; Data tested on EEBOT 102934
BASE_LINE     FCB   $D0        ; all white / all black reading
BASE_BOW      FCB   $22        ; slightly above all white
BASE_MID      FCB   $EA        
BASE_PORT     FCB   $EC        
BASE_STBD     FCB   $E1        

LINE_VARIANCE FCB   $10           ; Adding variance based on testing
...
;***************************************************************************************************
SENSOR_LINE   FCB   $01                     ; Sensor locations in mem
SENSOR_BOW    FCB   $23                     
SENSOR_PORT   FCB   $45
SENSOR_MID    FCB   $67
SENSOR_STBD   FCB   $89
...
_Startup: 
              ...
              LDS   #$4000                 ; Init stack
              CLI                          ; Init interrupts (I dont think we use these, double check)
              JSR   INIT                   ; Init ports
              JSR   openADC                ; Init ATD
              JSR   initLCD                ; Init LCD
              JSR   CLR_LCD_BUF            ; Write space to lcd
              BSET  DDRA,%00000011         ; STAR_DIR, PORT_DIR                        
              BSET  DDRT,%00110000         ; STAR_SPEED, PORT_SPEED                    
              JSR   initAD                 ; Init ATD converter                    
...
MAIN        
              ...
              LDAA  CRNT_STATE         
              JSR   DISPATCHER             ; Run dispatcher
              BRA   MAIN                   ; Repeat to inf
...
FWD_ST            BRSET   PORTAD0, $04, NO_FWD_BUMP           ; If bow bumper not hit, go to NO_FWD_BUMP                        
                  MOVB    #REV_TRN, CRNT_STATE                          
                  JSR     UPDT_DISPL
                  JSR     INIT_REV                            ; If bump, reverse, wait a bit, then start turning right, wait, exit                                     
                  LDY     #6000                                                                   
                  JSR     del_50us                                                                
                  JSR     INIT_RIGHT                                                              
                  LDY     #6000                                                                   
...
NO_FWD_BUMP       BRSET   PORTAD0, $04, NO_FWD_REAR_BUMP      ; work here
                  MOVB    #ALL_STOP, CRNT_STATE                                  
                  JSR     INIT_STOP                           
                  LBRA    EXIT 
                  
NO_FWD_REAR_BUMP  LDAA    SENSOR_BOW                          ; If the bow sensor or the mid sensor                         
                  CMPA    BASE_BOW                            ; do not detect black, assume we are                                    
                  BPL     NOT_ALIGNED                         ; not in alignment                                       
                  LDAA    SENSOR_MID                          ; If so, go to NOT_ALIGNED (Turn left)                            
                  CMPA    BASE_MID                                                                
                  BPL     NOT_ALIGNED                                                               
                  LDAA    SENSOR_LINE                                                             
                  ADDA    LINE_VARIANCE                                                                
...
NOT_ALIGNED       LDAA    SENSOR_PORT                         ; If there is a left path, turn left                            
                  CMPA    BASE_PORT                           ; Otherwise,                                   
                  BPL     PARTIAL_LEFT_TRN                                                        
                  BMI     NO_PORT                                                             

NO_PORT           LDAA    SENSOR_BOW                          ; If there is a straight path, go straight                         
                  CMPA    BASE_BOW                            ; Otherwise,                                    
                  BPL     EXIT                                                                    
                  BMI     NO_BOW                                                              

NO_BOW            LDAA    SENSOR_STBD                         ; If there is a right path,                             
                  CMPA    BASE_STBD                           ; Go right.                                    
                  BPL     PARTIAL_RIGHT_TRN                                                         
                  BMI     EXIT 

;***************************************************************************************************

PARTIAL_LEFT_TRN  LDY     #6000                              ; Turn left by a fixed amount                                   
                  jsr     del_50us                                                                
                  JSR     INIT_LEFT                                                               
...
CHECK_LEFT_ALIGN  JSR     INIT_LEFT                            ; Turn left indefinitely (Until dispatcher stops checking left align)                                  
                  MOVB    #LEFT_ALIGN, CRNT_STATE                                                 
                  BRA     EXIT

;*************************************************************************************************** 

PARTIAL_RIGHT_TRN LDY     #6000                               ; See above                                   
                  jsr     del_50us                                                                
                  JSR     INIT_RIGHT                                                              
...
CHECK_RIGHT_ALIGN JSR     INIT_RIGHT                             ; See above                                 
