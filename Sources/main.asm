...
NOT_FWD       CMPA  #REV                  ; Else if it's the REVERSE state            E
              BNE   NOT_REV               ;                                           R
              JSR   REV_ST                ; then call REV_ST routine                  |
              JMP   DISP_EXIT             ; and exit                                  |
                                          ;                                           |
NOT_REV       CMPA  #ALL_STP              ; Else if it's the ALL_STP routine          |
              BNE   NOT_ALL_STP           ;                                           |
              JSR   ALL_STP_ST            ; then call the ALL_STP state               |
              JMP   DISP_EXIT             ; and exit                                  |
                                          ;                                           |
NOT_ALL_STP   CMPA  #FWD_TRN              ; Else if it's the FORWARD_TURN state       |
              BNE   NOT_FWD_TRN           ;                                           |
              JSR   FWD_TRN_ST            ; then call the FWD_TURN state              |
              JMP   DISP_EXIT             ; and exit                                  |   
                                          ;                                           |
...
TOF_ISR     INC   TOF_COUNTER
            LDAA  #%10000000    ; Clears the TOF flag by setting bit 7
            STAA  TFLG2         ; TOF
            RTI
...
UPDT_DISPL   
             ; Display the battery voltage
              ...
              LDAA  ATDDR0L               ;load the ch0 result into AccA
              LDAB  #39                   ;AccB = 39
